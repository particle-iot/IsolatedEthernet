...
class Stream : public Print
{
  ...
  public:
     ...
     *
     * For files, buffers, or packets, the number of bytes left to read in the file from the current file position.
     */
     ...
     * @return A byte value 0 - 255, or -1 if there are no bytes available.
     *
     * For files, buffers, or packets, the current file position is left unchanged.
     */
    virtual int peek() = 0;
...
// parsing methods
   ...
   * @param target The string to search for. The string is specified by length so it does not need to be null-terminated.
   *
   * @param targetLen The number of bytes in target to search for.
   *
   * @param terminate The terminator string to search for. The string is specified by length so it does not need to be null-terminated.
   *
